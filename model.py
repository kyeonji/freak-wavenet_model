...
def WaveNetResidualConv1D(num_filters, kernel_size, dilation_rate):
    ...
    def build_residual_block(l_input):
        ...
        l_mul = GroupNormalization(groups=1)(l_mul)
        l_skip_connection = tf.keras.layers.Conv1D(1, 1, padding="causal", groups=4)(l_mul)
...
def build_wavenet_model(input_size, num_channel, num_filters, kernel_size,
    ...
    for i in range(num_residual_blocks):
        l_stack_conv1d, l_skip_connection = WaveNetResidualConv1D(
            num_filters, kernel_size, 2 ** i)(l_stack_conv1d)
...
def load_wavenet_model(input_size, num_channel, num_filters, kernel_size,
                        num_residual_blocks):
    """ Returns an implementation of WaveNet, as described in Section 2
        of the paper [1].
        Args:
    ...
    relu = Activation("relu")(l_sum)
    relu = GroupNormalization(groups=1)(relu)
    l1_conv1d = tf.keras.layers.Conv1D(num_channel, 1, activation="relu", padding="causal")(relu)
    l1_conv1d = GroupNormalization(groups=1)(l1_conv1d)
    l2_conv1d = tf.keras.layers.Conv1D(num_channel, 1, padding="causal")(l1_conv1d)
    # l_flatten = Flatten()(l2_conv1d)
    # l_output = Dense(128)(l_flatten)
    l2_conv1d = l2_conv1d[:, 7:-1, :]
    model = Model(inputs=[l_input], outputs=[l2_conv1d])
